...
private:
    ...
    double min_rpm = 13000;
    double max_rpm = 22000;
    double g = 9.81;
    double z_intergral_val = 0;
    double z_intergral_limit = 1.6;
    double RPM_change_limit = 1000;
    double Torque_limit = 12;
...
cascadePID::cascadePID(double control_rate)
{
    ...
    RPM_change_limit = 700.0*(dt/0.005);
...
{
    if (!pos_fb.allFinite() || !vel_fb.allFinite() || !q_in.coeffs().allFinite() || (q_in.norm() < 1e-6))
    {
        return;
    }
...
void cascadePID::RunController()
{
    ...
    // Altitude hold guard: keep enough lift authority near ground.
    Pos_des(2) = clamp(Pos_des(2), 0.8, 2.2);
    if (current_pos(2) < 0.45)
    {
        Pos_des(2) = std::max(Pos_des(2), 1.05);
    }
    Pos_error = Pos_des - current_pos;
    Vel_error = Vel_des - current_vel;

    double max_pos_error = 6.0;//*dt/(1.0/20.0)
    double max_vel_error = 6.0;//*dt/(1.0/20.0)
    ...
    Vector3d z_body;
    z_body << 0,0,1;
    double tilt_cos = clamp((R_body2world * z_body).dot(z_body), -1.0, 1.0);
    double tilt_denom = std::max(0.35, std::abs(tilt_cos));
    double F_des = F_des_z / tilt_denom;

    // std::cout << "F_des_z = " << F_des_z << " "<< "F_des = " << F_des << std::endl;

    F_des = std::max(std::min(F_des, mass*2.0*g), mass*1.0*g);
    ...
    acc_x_des = std::max(std::min(acc_x_des, 1.2), -1.2);
    acc_y_des = std::max(std::min(acc_y_des, 1.2), -1.2);
    ...
    double new_z_norm = new_z_body.norm();
    if (!std::isfinite(new_z_norm) || new_z_norm < 1e-6)
    {
        new_z_body = z_body;
    }else{
        new_z_body = new_z_body / new_z_norm;
    }
    Matrix3d new_z_body_inv,R_des,I;
    //if at a line
    double zz_dot = clamp(new_z_body.dot(z_body), -1.0, 1.0);
    if(zz_dot > 1.0 - 1e-6)
    {
    R_des << 1,0,0,
            0,1,0,
            0,0,1;
    }else if(zz_dot < -1.0 + 1e-6){
    R_des << -1,0,0,
             0,1,0,
             0,0,-1;
    }else{
    rotation_vec = z_body.cross(new_z_body);
    double rv_norm = rotation_vec.norm();
    if (!std::isfinite(rv_norm) || rv_norm < 1e-6)
    {
        R_des << 1,0,0,
                0,1,0,
                0,0,1;
    }else{
    rotation_vec = rotation_vec / rv_norm;

    new_z_body_inv << 0,-rotation_vec(2),rotation_vec(1),
                        rotation_vec(2),0,-rotation_vec(0),
                        -rotation_vec(1),rotation_vec(0),0;
    double rotation_angle = acos(zz_dot);
    rotation_angle = std::max(std::min(rotation_angle, 0.16), -0.16);
    I << 1,0,0,
        0,1,0,
        0,0,1;
    ...
    Vector3d rotvec = rotation_vec_des.axis() * rotation_vec_des.angle();
    if (!rotvec.allFinite())
    {
        rotvec.setZero();
    }
    ...
    if(first_control_flag == 0)
    {
        ...
        d_Angle_error = (rotvec - Angle_last_error)/dt;
    }
    Angle_last_error = rotvec;

    Torque_des(0) =  (Angle_Kp(0)*rotvec(0) + Angle_Kd(0)*d_Angle_error(0));
    Torque_des(1) =  (Angle_Kp(1)*rotvec(1) + Angle_Kd(1)*d_Angle_error(1));
    Torque_des(2) =  (Angle_Kp(2)*rotvec(2) + Angle_Kd(2)*d_Angle_error(2));
    Torque_des = Internal_mat * Torque_des;
    for (int i = 0; i < 3; i++)
    {
        Torque_des(i) = finite_or(Torque_des(i), 0.0);
    }
    ...
    {
        // sqrt cause nan
        double m1 = Mixermatrix_rpm2torque(1,i);
        double m2 = Mixermatrix_rpm2torque(2,i);
        double m3 = Mixermatrix_rpm2torque(3,i);
        if (std::abs(m1) < 1e-12) { m1 = (m1 >= 0.0) ? 1e-12 : -1e-12; }
        if (std::abs(m2) < 1e-12) { m2 = (m2 >= 0.0) ? 1e-12 : -1e-12; }
        if (std::abs(m3) < 1e-12) { m3 = (m3 >= 0.0) ? 1e-12 : -1e-12; }
        double rpm_sq = finite_or(F_des, mass * g)/(4.0*k_F)
                        + Torque_des(0)/(4.0 * m1)
                        + Torque_des(1)/(4.0 * m2)
                        + Torque_des(2)/(4.0 * m3);
        ...
        if(!std::isfinite(rpm_sq) || rpm_sq < 0)
        {
            RPM_output(i) = 0;
        }else{
            RPM_output(i) = sqrt(rpm_sq);
        }
    }
    //limit
    for(int i = 0;i<4;i++)
